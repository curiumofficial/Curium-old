...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Curium version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  curiumd [options]                     " + "\n" +
                  "  curiumd [options] <command> [params]  " + _("Send command to -server or curiumd") + "\n" +
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: curium.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: curiumd.pid)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 9364 or testnet: 19364)") + "\n" +
        ...
        "  -anonymizecuriumamount=<n>      "   + _("Keep N curium anonymized (default: 0)") + "\n" +
        ...
        "\n" + _("SSL options: (see the Curium Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Curium is probably already running."), strDataDir.c_str()));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Curium") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Curium to complete") << "\n";
    ...
    nAnonymizeCuriumAmount = GetArg("-anonymizecuriumamount", 0);
    if(nAnonymizeCuriumAmount > 999999) nAnonymizeCuriumAmount = 999999;
    if(nAnonymizeCuriumAmount < 2) nAnonymizeCuriumAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeCuriumAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
