...
void OverviewPage::updateDarksendProgress(){
    ...
    double max = nAnonymizeCuriumAmount;
    ...
    if(nBestHeight != darkSendPool.cachedNumBlocks)
    {
        ...
        if (pwalletMain->GetBalance() - pwalletMain->GetAnonymizedBalance() > 2*COIN){
            if (walletModel->getEncryptionStatus() != WalletModel::Unencrypted){
                if((nAnonymizeCuriumAmount*COIN)-pwalletMain->GetAnonymizedBalance() > 1.1*COIN && walletModel->getEncryptionStatus() == WalletModel::Locked){
...
void OverviewPage::toggleDarksend(){
    ...
    if(balance < 2.5*COIN){
        QMessageBox::warning(this, tr("Darksend"),
            tr("Darksend requires at least 2.5 CRU to use."),
