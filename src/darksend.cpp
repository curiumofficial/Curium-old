...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    // if we have more denominated funds (of any maturity) than the nAnonymizeCuriumAmount, we should use use those
    if(pwalletMain->GetDenominatedBalance(true) >= nAnonymizeCuriumAmount*COIN) {
        minRounds = 0;
    }
    //if we're set to less than a thousand, don't submit for than that to the pool
    if(nAnonymizeCuriumAmount < 1000) maxAmount = nAnonymizeCuriumAmount;
    ...
    // the darksend pool can only take 1.1CRU minimum
    if(nValueIn < COIN*1.1){
        ...
        LogPrintf("DoAutomaticDenominating : Too little to denominate (must have 1.1CRU) \n");
        return false;
    }
    ...
    if(!sessionFoundMasternode){
        ...
        double fCuriumSubmitted = sessionTotalValue / COIN;

        LogPrintf("Submiting Darksend for %f CRU\n", fCuriumSubmitted);
...
int CDarkSendPool::GetDenominations(const std::vector<CTxOut>& vout){
    ...
    //
    // bit 0 - 500CRU+1 ( bit on if present )
    // bit 1 - 100CRU+1 
    // bit 2 - 10CRU+1 
    // bit 3 - 1CRU+1 
    // bit 4 - fee
...
// calculate the outputs from a given amount of curium and 
