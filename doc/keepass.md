...
The following new options are available for curiumd and curium-qt:
...
The following rpc commands are available:
 ...
 - _keepass init_: sets up the association between curiumd and keepass by generating an AES key and sending an association message to KeePassHttp. This will trigger KeePass to ask for an Id for the association. Returns the association and the base64 encoded string for the AES key.
...
Sample initialization flow from _curium-qt_ console (this needs to be done only once to set up the association):
...
Extended guide with screenshots is also available: https://curiumtalk.org/threads/keepass-integration.3620/
